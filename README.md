...
- 7.1 编写了防止缓存击穿的singleflight文件，并保证并发安全
- 其中的Group作为GeeCache的主要数据结构中的变量
- singleflight文件中的Group用来限制每一个key的请求,使用写锁mu.Lock()保证同一时刻内只有一个call被创建
- 用WaitGroup保证重复的请求不会再次执行fn,而是等待wait第一个fn执行完毕返回的数据
- 加锁时机:当一个变量是共享变量,且需要保证同一时刻只有一个goroutine对变量进行读/写(需要保证变量不被其他goroutine修改/不修改其他goroutine正在读取的变量时)的时候mu.Lock()
- 解锁时机:不再对加锁的变量进行读写的时候